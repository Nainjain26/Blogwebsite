...
const BlogSection = () => {
  ...
  const [sidePosts, setSidePosts] = useState(initialSidePosts);
  const [textPosts, setTextPosts] = useState(initialTextPosts);
  const [hasMore, setHasMore] = useState(true); // Manage "view all" state
  ...
  const loadMoreContent = () => {
    ...
    // Set hasMore to false to disable further loading
    setHasMore(false);
  };
