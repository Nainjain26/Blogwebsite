    ...
    <div className="bg-white text-black px-4 py-8 scroll-smooth">
      ...
      <nav aria-label="Breadcrumb" className="max-w-7xl mx-auto">
        <div className="text-sm text-gray-500 flex items-center justify-between">
          <div>
            <Link to="/blogs" className="text-gray-500 font-semibold hover:text-yellow-500 transition-colors">
              Blogs
            </Link>
      ...
      <div className="p-2 bg-[#fafafa]">
        <div className="mt-8 grid md:grid-cols-4 gap-10 max-w-7xl mx-auto" ref={articleRef}>
          {/* Sidebar */}
          <aside className="md:col-span-1 text-sm sticky top-24 self-start">
          ...
          {/* Article Body */}
          <article className="md:col-span-3 space-y-10 text-base">
            {sections.map((section, idx) => (
              <section
                key={section.id}
                id={section.id}
                ref={(el) => (sectionRefs.current[idx] = el)}
                className="scroll-mt-24 bg-white p-6 rounded-lg shadow-sm"
              >
                ...
                {section.hasStats && (
                  <div className="grid grid-cols-2 gap-4 my-6">
                    {section.stats.map((stat, l) => (
                      <div key={l} className="bg-gray-50 p-4 rounded-lg text-center">
